...
switch($action) {
	...
	case "load":
		$query 	= mysql_query("SELECT * FROM `req_config` ORDER BY id ASC");
        ...
        <table class="as_gridder_table table">
            <tr class="grid_header">
                <td><div class="grid_heading">Request Type</div></td>
	...
	case "update":
		...
		$rowId = $explode[1];
		$query = mysql_query("UPDATE `req_config` SET `$columnName` = '$value' WHERE id = '$rowId' ");
	break;
