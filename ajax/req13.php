...
if($_POST['act'] == 'req'.$idpost):
    ...
    $studentname = htmlentities($studentname);
    $files = htmlentities($files);
    $reason = htmlentities($reason);
    $duration = htmlentities($duration);
    $studenttype = htmlentities($studenttype);
    $facultyname = htmlentities($facultyname);
    $facultyemail = htmlentities($facultyemail);
    
    $message = 'Name of student: '.$studentname.
    			',Student Resume: '.$files.
    			',Reason why the student should be granted a scholarship: '.$reason.
    			',Duration of scholarship: '.$duration.
    			', Student Type: '.$studenttype.
    			',Faculty Contact Name:'.$facultyname.
