...
if($_POST['act'] == 'contr'.$idpost):
    ...
    $milestonedesc = htmlentities($milestonedesc);
    $projectname = htmlentities($projectname);
    $milestonesigni = htmlentities($milestonesigni);
    $medialink = htmlentities($medialink);
    $contactname = htmlentities($contactname);
    $contactemail = htmlentities($contactemail);
    
    $message = 'Description of milestone achieved: '.$milestonedesc.
    			', Name of Project (if applicable): '.$projectname.
    			', Significance of milestone: '.$milestonesigni.
    			', Link to media mentions: '.$medialink.
