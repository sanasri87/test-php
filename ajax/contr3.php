...
if($_POST['act'] == 'contr'.$idpost):
    ...
    $quotedname = htmlentities($quotedname);
    $articlelink = htmlentities($articlelink);
    $articlecontext = htmlentities($articlecontext);
    $contactname = htmlentities($contactname);
    $contactemail = htmlentities($contactemail);
    
    $message = 	'Name of person quoted in the media: '.$quotedname.
    ', Link to article: '.$articlelink.
    ', Context of article: '.$articlecontext.
