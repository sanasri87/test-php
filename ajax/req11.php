...
if($_POST['act'] == 'req'.$idpost):
    ...
    $handoverlocation = htmlentities($handoverlocation);
    $handoverdate = htmlentities($handoverdate);
    $projectname = htmlentities($projectname);
    $contactname = htmlentities($contactname);
    $contactemail = htmlentities($contactemail);
    
    $message = 'Location of space being handed over: '.$handoverlocation.
    			', Date when the space can be handed over: '.$handoverdate.
    			',Project for which the space was required: '.$projectname.
